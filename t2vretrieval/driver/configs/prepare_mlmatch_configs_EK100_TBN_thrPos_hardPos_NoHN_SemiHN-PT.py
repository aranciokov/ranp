...
def prepare_match_model(root_dir):
  ...
  output_dir = os.path.join(res_dir, 'mlmatch', 
    'ek100_TBN_HP_NoHN_SemiHN-PT_m%s.mpos%s.vis.%s%s.txt.%s.%d.loss.%s.af.%s.%d%s.glove.init.%dep.thrPos%s'%
    (model_cfg.margin, model_cfg.margin_pos, '-'.join(attn_ft_names),
