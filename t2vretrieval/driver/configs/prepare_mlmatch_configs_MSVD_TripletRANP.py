...
def prepare_match_model(root_dir):
  anno_dir = os.path.join(root_dir, 'annotation', 'msvdRET')
  attn_ft_dir = os.path.join(root_dir, 'ordered_feature', 'MSVD')
  split_dir = os.path.join(root_dir, 'public_split_msvd')
  ...
  num_words = len(np.load(os.path.join(anno_dir, 'int2word_msvd.npy')))

  model_cfg = t2vretrieval.models.mlmatch.RoleGraphMatchModelConfig()
  model_cfg.threshold_pos = 0.10
  ...
  output_dir = os.path.join(res_dir, 'mlmatch', 
    'msvd_m%s%sHP_mPos%s.vis.%s%s.txt.%s.%d.loss.%s.af.%s.%d%s.glove.init.%dep.thrPos%s'%
  ...
  path_cfg = {
    ...
    'attn_ft_files': {},
    'name_file': {},
    'word2int_file': os.path.join(anno_dir, 'word2int_msvd.json'),
    'int2word_file': os.path.join(anno_dir, 'int2word_msvd.npy'),
    'ref_caption_file': {},
    'dataset_file': {},
    'ref_graph_file': {},
    'val_relevance_file': os.path.join(anno_dir, 'msvd_val_relevance.pkl'),
    'relevance_file': os.path.join(anno_dir, 'msvd_test_relevance.pkl')
  }
  for setname in ['trn', 'val', 'tst']:
    ...
    if setname == 'trn':
      path_cfg['dataset_file'][setname] = os.path.join(anno_dir, 'msvd_train.json')
