...
def prepare_match_model(root_dir):
  ...
  model_cfg.semi_hard_neg = True
  model_cfg.use_word2vec = True
  model_cfg.use_normsoftmax = True
  ...
  output_dir = os.path.join(res_dir, 'eaomatch', 
    'charades_EAO_baseSemiHN_m%s.vis.%s.txt.%s.%d.%dheads.%s%sProj%d.MLPr%.1f.loss.%s.af.%s.%d%s.w2v.init.%dep'%
