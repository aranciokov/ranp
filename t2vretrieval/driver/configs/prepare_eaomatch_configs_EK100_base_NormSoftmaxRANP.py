...
def prepare_match_model(root_dir):
  ...
  output_dir = os.path.join(res_dir, 'eaomatch', 
    'ek100_NoHN_NormSoftmaxRANPLoss.tmp%s.thr%s.vis.%s.txt.%s.%d.%dheads.%s%sProj%d.MLPr%.1f.af.%s.%d%s.w2v.init.%dep'%
    (model_cfg.temperature, model_cfg.threshold_pos, '-'.join(attn_ft_names),
      txtenc_name, 
