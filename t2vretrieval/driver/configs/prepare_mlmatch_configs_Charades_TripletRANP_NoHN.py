...
def prepare_match_model(root_dir):
  anno_dir = os.path.join(root_dir, 'annotation', 'charadesRET')
  attn_ft_dir = os.path.join(root_dir, 'ordered_feature', 'Charades')
  split_dir = os.path.join(root_dir, 'public_split_charades')
  res_dir = os.path.join(root_dir, 'results', 'RET.released')
  
  attn_ft_names = ['ViT']  # RegNet, EffNet  -> set dim_fts to: ViT->1280, RegNet->7392, EffNet->1280
  num_words = len(np.load(os.path.join(anno_dir, 'int2word_charades.npy')))

  model_cfg = t2vretrieval.models.mlmatch.RoleGraphMatchModelConfig()
  model_cfg.threshold_pos = 0.05
  model_cfg.mine_hard_positives = True
  model_cfg.margin_pos = 0.2
  model_cfg.temperature = 0.05
  ...
  output_dir = os.path.join(res_dir, 'mlmatch', 
    'charades_m%s%sHP_NoHN_mPos%s.vis.%s%s.txt.%s.%d.loss.%s.af.%s.%d%s.glove.init.%dep.thrPos%s'%
    (model_cfg.margin, '_' if model_cfg.max_violation else '_noHN_', model_cfg.margin_pos, '-'.join(attn_ft_names),
     ...
     model_cfg.num_epoch, model_cfg.threshold_pos
      )
    )
  print(output_dir)
  ...
  path_cfg = {
    ...
    'attn_ft_files': {},
    'name_file': {},
    'word2int_file': os.path.join(anno_dir, 'word2int_charades.json'),
    'int2word_file': os.path.join(anno_dir, 'int2word_charades.npy'),
    'ref_caption_file': {},
    'dataset_file': {},
    'ref_graph_file': {},
    'val_relevance_file': os.path.join(anno_dir, 'charades_test_relevance_SYN.pkl'),
    'relevance_file': os.path.join(anno_dir, 'charades_test_relevance_SYN.pkl')
  }
  for setname in ['trn', 'val', 'tst']:
    ...
    if setname == 'trn':
      path_cfg['dataset_file'][setname] = os.path.join(anno_dir, 'charades_train.json')
