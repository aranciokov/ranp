...
def prepare_match_model(root_dir):
  ...
  model_cfg.use_normsoftmax_ranp = True
  ...
  output_dir = os.path.join(res_dir, 'eaomatch', 
    'charades_EAO_SemiHNPT_RANPmPos%sthr%s_WithHN_m%s.vis.%s.txt.%s.%d.%dheads.%s%sProj%d.MLPr%.1f.loss.%s.af.%s.%d%s.w2v.init.%dep'%
    (model_cfg.margin_pos, model_cfg.threshold_pos, model_cfg.margin, '-'.join(attn_ft_names),
      txtenc_name, 
