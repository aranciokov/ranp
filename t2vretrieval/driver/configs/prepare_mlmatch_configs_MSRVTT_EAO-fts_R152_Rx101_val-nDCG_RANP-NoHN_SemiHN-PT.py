...
def prepare_match_model(root_dir):
  ...
  output_dir = os.path.join(res_dir, 'mlmatch', 
    'msr_ndcgval_RANP.thr%s-NoHN_SemiHN-PT.vis.%s%s.txt.%s.%d.loss.%s.af.%s.%d%s.glove.init.%dep.%dbs'%
    (model_cfg.threshold_pos, '-'.join(attn_ft_names),
     ...
     model_cfg.num_epoch, model_cfg.trn_batch_size
      )
    )
  print(output_dir)
